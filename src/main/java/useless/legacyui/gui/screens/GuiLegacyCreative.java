...
public class GuiLegacyCreative extends ScreenInventory implements IGuiController {
    ...
    public GuiLegacyCreative(final Player player) {
        super(player);
        this.player = player;
        container = (LegacyContainerPlayerCreative)player.containerMenu;
    }
    ...
    public void render(final int mx, final int my, final float partialTick) {
        ...
        this.mc.textureManager.loadTexture(useless.legacyui.settings.LegacySettings.guiTexture("legacycreative")).bind();
        ...
        drawStringCenteredShadow(this.fontRenderer, this.clearButton.displayString, this.clearButton.xPosition + (this.clearButton.width/2), this.clearButton.yPosition + 6, LegacySettings.GUI_PROMPT_COLOR.value.value);
    ...
    protected void drawGuiContainerBackgroundLayer(final float renderPartialTick) {
        ...
        drawStringCenteredNoShadow(this.fontRenderer, LegacyCategoryManager.getCreativeCategories().get(currentTab).getTranslatedKey(), GUIx + this.xSize /2, GUIy + 32, useless.legacyui.settings.LegacySettings.guiTextColor());
    }
