...
public class GuiLegacyInventory extends ScreenInventory implements IGuiController {
    ...
    public GuiLegacyInventory(final Player player) {
        ...
        this.player = player;
        this.inventorySlots = Gamemodes.SURVIVAL.getInventoryMenu(player.inventory);
    }
    public void init() {
        super.init();
        ...
        if(this.player.getGamemode() == Gamemodes.CREATIVE){
    ...
    public void render(final int mx, final int my, final float partialTick) {
        ...
        mc.textureManager.loadTexture(useless.legacyui.settings.LegacySettings.guiTexture("legacyinventory")).bind();
    ...
    protected void drawGuiContainerBackgroundLayer(final float renderPartialTick) {
        ...
        renderPlayerDoll();
        drawStringNoShadow(this.fontRenderer, I18n.getInstance().translateKey("legacyui.guilabel.inventory"),GUIx + 8,GUIy +82, useless.legacyui.settings.LegacySettings.guiTextColor());
    }
    private void renderPlayerDoll(){
        GLRenderer.enableState(State.DEPTH_TEST);
        GLRenderer.pushFrame();

        // Clip anything drawn below to the character-preview box so things like the entity's
        // ground shadow decal can't spill out onto the surrounding GUI panel.
        final int guiScale = this.mc.resolution.getScale();
        final int boxX = GUIx + 55;
        final int boxYTop = GUIy;
        final int boxWidth = 80;
        final int boxHeight = 82;
        final int windowHeight = this.mc.resolution.getHeightScreenCoords();
        final int scissorX = boxX * guiScale;
        final int scissorY = windowHeight - (boxYTop + boxHeight) * guiScale;
        final int scissorW = boxWidth * guiScale;
        final int scissorH = boxHeight * guiScale;
        GL11.glEnable(GL11.GL_SCISSOR_TEST);
        GL11.glScissor(scissorX, scissorY, scissorW, scissorH);

        GLRenderer.modelM4f().translate(GUIx + 51 + 44, GUIy + 75 - 5, 50.0f);
        ...
        GLRenderer.modelM4f().rotate((float) Math.toRadians(135.0f), 0.0f, 1.0f, 0.0f);
        Lighting.enableLight();
        GLRenderer.modelM4f().rotate((float) Math.toRadians(-135.0f), 0.0f, 1.0f, 0.0f);
        GLRenderer.modelM4f().rotate((float) Math.toRadians(-((float)Math.atan(f6 / 40.0f)) * 20.0f), 1.0f, 0.0f, 0.0f);
        ...
        EntityRendererDispatcher.instance.renderEntityWithPosYaw(GLRenderer.getTessellator(), this.mc.thePlayer, 0.0, 0.0, 0.0, 0.0f, 1.0f);
    ...
    public void guiSpecificControllerInput(final ControllerInput controllerInput) {
        ...
        if (controllerInput.buttonLeftTrigger.pressedThisFrame() && this.player.getGamemode() == Gamemodes.CREATIVE){
            openCreative();
        }
