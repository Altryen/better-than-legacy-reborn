...
public class LegacyContainerPlayerSurvival extends MenuInventory {
    public LegacyContainerPlayerSurvival(final ContainerInventory inventory) {
        super(inventory);
        ...
        for (int index = 0; index < 4; ++index) { // Create Armor Slots
            this.addSlot(new SlotArmor(this, inventory, inventory.getContainerSize() - 1 - index, 49, 8 + (3 - index) * 18, HumanArmorShape.values()[3 - index]));
        }
    ...
    @Override
    public IntList getTargetSlots(final InventoryAction inventoryAction, final Slot slot, final int target, final Player entityPlayer) {
        if (slot.index >= 9 && slot.index <= 44) { // Entire Inventory
