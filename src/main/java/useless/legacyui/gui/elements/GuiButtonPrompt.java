...
public class GuiButtonPrompt extends GuiRegion implements GuiElement {
    ...
    public GuiButtonPrompt(int id, int xPosition, int yPosition, int spacing, String prompt, int[] buttonCoordinates) {
        ...
        this.width = (buttonCoordinates.length * 13) + (spacing * buttonCoordinates.length) + fontRenderer.stringWidth(prompt);
        this.height = 13;
    }
    public void drawPrompt(Minecraft minecraft, int mouseX, int mouseY){
        if (!LegacySettings.FORCE_BUTTON_PROMPTS.value && minecraft.inputType != InputType.CONTROLLER) {return;}
        ...
        int v = LegacySettings.GUI_CONTROLLER_TYPE.value.index()  * 13;
        ...
        fontRenderer.render(prompt, xPosition + ((13 + spacing) * buttonCoordinates.length), yPosition+2).setColor(LegacySettings.GUI_PROMPT_COLOR.value.value).call();
    }
}
