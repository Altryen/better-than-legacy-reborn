...
public class ItemCategoryBuilder {
    ...
    private static final List<ItemStack> allCreativeItems = MenuInventoryCreative.creativeContents;
    ...
    public ItemCategory build(){
        ...
        final List<ItemStack> orderedCategoryItems = new ArrayList<>(categoryItems);
        UtilSorting.reorderItemStacksByExplicitPriority(orderedCategoryItems, this.exclusiveItemList);
        UtilSorting.sortItemStacksByMaterialTierInPlace(orderedCategoryItems);
        final ItemStack[] returnArray = new ItemStack[orderedCategoryItems.size()];
        for (int i = 0; i < returnArray.length; i++) {
            returnArray[i] = orderedCategoryItems.get(i);
        }
